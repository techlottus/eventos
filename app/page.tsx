...
export default async function Home() {
  ...
  const cardListData = eventHomeData?.activeEvents?.data;
  const finishedEvents = eventHomeData?.finishedEvents?.data;
    ...
    <HeaderFooterLayout breadcrumbs={false} layoutData={layoutData}>
      <div className="flex justify-center w-full">
        <section className="desktop:mx-21 tablet:mx-10 mobile:mx-6 mx-auto max-w-[1200px]">
          ...
          <div id="events-finished" className="flex flex-col gap-6 justify-center items-center mt-6">
            <h1 className="font-headings text-5xl font-bold text-surface-900">Eventos finalizados</h1>
            <div id="card-list-finished" className="grid grid-cols-4 gap-6 mt-6 h-fit">
              {finishedEvents?.map((cardData: CardType, i: number) => (
                <Card key={i} {...cardData} />
              ))}
            </div>
          </div>
